import pigpio
from PuppyPI_Config import ServoParams, PWMParams
        ...
        for leg_index in range(4):
            for axis_index in range(3):
                self.pi.set_PWM_frequency(
                    self.pwm_params.pins[axis_index, leg_index], self.pwm_params.freq
                )
                self.pi.set_PWM_range(self.pwm_params.pins[axis_index, leg_index], self.pwm_params.range)
